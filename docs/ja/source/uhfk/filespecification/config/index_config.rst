...
- ``BoundaryCondition``
  ...
  **形式 :**
  長さ 3 の文字列リスト (デフォルト値は ``["periodic", "periodic", "periodic"]``)

  **説明 :**
  各空間方向 (x, y, z) の境界条件を指定するパラメータです。
  各方向に指定できる値は ``"periodic"`` / ``"P"`` および ``"antiperiodic"`` / ``"AP"``
  (大文字小文字無視)です。
  反周期境界条件はツイスト角 :math:`\pi` に対応します。

  v1 では、 ``BoundaryCondition`` に ``"antiperiodic"`` を含む場合、
  ``SubShape = [1, 1, 1]`` を明示的に指定する必要があります。
  ``SubShape`` のデフォルトは ``CellShape`` であり、反周期境界条件と組み合わせると例外となります。

  非密度型相互作用 ( ``PairHop`` , ``PairLift`` ) と反周期境界条件の併用は v1 では非対応です。
  これらはサイト依存ゲージ下でゲージ不変ではないためで、v2 で対応予定です。
